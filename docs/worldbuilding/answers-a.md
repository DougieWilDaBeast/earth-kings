# Answer sheet — Respondent A

Fill this in **alone**. Do not open `answers-b.md` until both sheets are done
and you are sitting down together. Questions in full:
[12 — The World Charter Interview](../12-world-charter-interview.md).

**Care tag** (put one in the brackets): `H` real stake · `M` preference · `L` shrug ·
`—` you decide · `!` veto (three for the whole interview).

Respondent: _name_ · Started: ______ · Finished: ______
...
**FR4** `[ ]` — What does "Earth Kings" mean? Who is a king, what is the Earth in that phrase, and is the title claimed, inherited, or given? Is the player one, hu…
...
**FR6** `[ ]` — The Codex permits "absurd results" as valid output. Is absurdity a truth about the world's underlying physics, or a joke shared with the player ove…

> 

**FR7** `[ ]` — A player just lost a character they cared about. What should they feel in the ten seconds after — grief, injustice, guilt, or the flat click of a r…
...
**SK6** `[ ]` — What is a step? Roughly how far, and how long does it take? Everything in the game is priced in steps — 900 to forget a book, 600 before a gate bre…
...
**CL1** `[ ]` — The seasons are named for clovers — Lesser Green, Green, Brown, Ice. Why a clover? Who named them, and does the name predate the game's world or be…
...
**LF9** `[ ]` — Are there plants, fungi or animals that only exist near a gate? What does long exposure do to farmland?
...
**MK11** `[ ]` — The bed at home grants permanent HP, up to +22 for a canopied one. What is that, physically? Is comfort literally strengthening, or is it rest, saf…
...
**GT8** `[ ]` — Gates never reopen, yet new rifts tear open under continental pressure (D26). Is there a fixed number? Is the world running out, or being slowly em…
...
**BD2** `[ ]` — Permadeath is the rule, but a grace buys survival — charm, ally, book, ground, luck. Is a grace a physical event, a moral one, or a law of the worl…
...
**KN8** `[ ]` — Doctrine text is aphoristic ("Sharpen at dusk, never at dawn"). Is that a *style*, a genre, or the only way this knowledge can be written down with…
...
**PW1** `[ ]` — What is the grammar? Skill trees are generated from a hidden system of themes and archetypes. Is that a law of nature, a language, an artefact, or…
...
**SF1** `[ ]` — Does the player character know they are unusual?
...
**SF3** `[ ]` — Is anyone watching the player's run from inside the fiction? (The lineage names Omniscient Reader's Viewpoint as an influence.)
...
**SF5** `[ ]` — The player *chooses* a class at level 2 and NPCs settle into one. Does a person in this world experience that as a choice, a discovery, or a diagno…
...
**SF7** `[ ]` — Is there prophecy or fate? If so, is it accurate, and does knowing it change it?
...
**RU13** `[ ]` — What happened to kings? The game is called Earth Kings and the factions are legions, courts, tides and wilds. Is kingship dead, ongoing, or the thi…
...
**LW3** `[ ]` — Is there such a thing as a trial? Who judges?
...
**LW5** `[ ]` — Captives are ransomed at a price, on a deadline, and may be sold if the deadline lapses. Sold to whom, for what work? Is slavery legal, normal, or…
...
**EC6** `[ ]` — Is there debt, credit, lending?
...
**KI6** `[ ]` — Every one of the twelve heroes is an orphan, an exile, a stray or a cloistered scholar who left. Is the rootless adventurer the norm, or is the gam…
...
**KI11** `[ ]` — Home is inviolable — nothing camps within sight of it, sleeping there always heals. Is that true of every home in the world, or only the player's?…
...
**DY2** `[ ]` — What do people drink, and where?
...
**DY5** `[ ]` — How do people fight for fun?
...
**SM1** `[ ]` — Save-scumming is allowed (D21) and permadeath is a pillar. Is reloading a fact about the world, a fact about the player, or a hole we agree never t…

> 

**SM2** `[ ]` — Difficulty settings change enemy strength, grace odds and whether permadeath exists at all. Are these different worlds, or the same world at differ…

> 

**SM3** `[ ]` — `Q` hands the run to the AI and it plays itself — walking, fighting, answering its own conversations. Is autoplay *anything* in the fiction, or pur…

> 

**SM4** `[ ]` — Two-player is passing the pad (D10). Since two of you are building this — is the second player a second character, a second run, or a person lookin…

> 

**SM5** `[ ]` — The run ends when the player character dies but the world is saved. Is the next run the same world later, a different one, or a reset? This is the…
...
**SM8** `[ ]` — Companions are hired for gold and dismissed at will, and D14 says the story continues when they die. How do we make a hire matter — is it fiction's…
...
**BN7** `[ ]` — If the other person's world won every disagreement, would you still want to build it? Be honest — this is the question that decides how the ledger…
