...
##
## An errand is one beat long: posted, walked, settled. A [Skein] is a chain of
...
##
## Named for a length of thread wound up; the game calls these threads, but
## Godot already owns that name.
##
...
## Keys that qualify the effect sitting beside them rather than being one.
const MODIFIERS := ["at", "weight"]
...
static func _stage_index(stages: Array, stage_id: String, fallback: int) -> int:
	...
	for i in stages.size():
		if String(stages[i].get("id", "")) == stage_id:
			return i
	push_warning("Skein: no stage named '%s'" % stage_id)
	return fallback
		...
		match key:
			...
			"thread_done":
				...
				if not bool(other.get("done", false)):
					return false
			_:
				push_warning("Skein: unknown condition '%s'" % key)
				return false
	return true
		...
		for key: String in effect:
			...
			match key:
				...
				_:
					...
					push_warning("Skein: unknown effect '%s' in '%s'" % [key, thread_id])
	return lines
