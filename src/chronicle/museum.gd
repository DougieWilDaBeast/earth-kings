...
static func compose(world: World, roster: Roster, ledger: Dictionary, ending: String) -> Dictionary:
	...
	return {
		...
		"steps": world.steps,
		"season": Season.current(world).get("display_name", "Spring"),
		"clover": Season.current(world).get("clover_name", "Lesser Green Clover"),
		"year": Season.year(world),
